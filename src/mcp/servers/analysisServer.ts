...
function extractDataElementsFromChanges(changes: unknown[]): string[] {
    if (!Array.isArray(changes)) return [];
    return changes
        .map((c: any) => String(c?.["Field Name"] ?? "").trim())
        .filter(Boolean);
}
...
server.registerTool(
    ...
    async ({ filePath, text, model }) => {
        ...
        const changes = (await chatStructured(llmModel, inputText)) as unknown[];
