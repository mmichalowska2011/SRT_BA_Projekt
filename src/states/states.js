...
const STATE_TEXT = {
  [STATES.INITIAL]: ({ lastSourceFile, lastChanges }) => {
  ...
  [STATES.ANALYZED]: ({ lastSourceFile, lastChanges }) => {
  ...
  [STATES.VALID]: ({ lastSourceFile, lastChanges }) => {
    const count = getCount(lastChanges);
    const fileInfo = lastSourceFile ? ` (Quelle: ${lastSourceFile})` : "";
...
export function getStateText({ state, lastChanges, lastSourceFile }) {
  const fn = STATE_TEXT[state];
  if (fn) return fn({ lastChanges, lastSourceFile });
